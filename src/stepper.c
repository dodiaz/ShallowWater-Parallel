...
static
void copy_in(float* restrict usub, float* restrict u,
	       	int nx, int ny, int ng, int nfield, double ncores)
{
    int nx_all = nx + 2*ng;
    int ny_all = ny + 2*ng;
    int S = nx_all * ny_all;
    ...
    double sqt_ncores = sqrt(ncores);
    double sqt_ncores_inv = 1/sqrt(ncores) ;

    int hnx = sqt_ncores_inv * nx;
    int hny = sqt_ncores_inv * ny;

    int hnx_all = hnx + 2*ng;
    int hny_all = hny + 2*ng;
    int s = hnx_all * hny_all;

    for (int k = 0; k < nfield ; ++k){
   	 for (int iy = 0; iy < hny_all; ++iy){
	     for (int ix = 0; ix < hnx_all; ++ix){
        	 usub[(k*hny_all+iy)*hnx_all + ix] = u[(k*ny_all+iy)*nx_all + ix];
	    }
    	}
    }
}
...
static
void copy_out(float* restrict usub, float* restrict u,
	       	int nx, int ny, int ng, int nfield, double ncores)
{
    int nx_all = nx + 2*ng;
    int ny_all = ny + 2*ng;
    ...
    for (int k = 0; k < nfield ; ++k){
   	 for (int iy = ng; iy < hny_all - ng; ++iy){
	    for (int ix =  ng ; ix < hnx_all - ng; ++ix){
                u[(k*ny_all+iy)*nx_all + ix] = usub[(k*hny_all + iy)*hnx_all + ix];
	    }
    	}
    }
}
                   ...
                   float tfinal, float dx, float dy, float cfl)
{
    ...
    double ncores = 4.0;
    double sqt_ncores = sqrt(ncores);
    double sqt_ncores_inv = 1/sqt_ncores;
    ...
    float* usub = (float*) malloc(ncores*(4*nfield*s + 6*hnx_all)  * sizeof(float) );
	...
	central2d_periodic(u, nx, ny, ng, nfield);
        speed(cxy, u, nx_all * ny_all, nx_all * hny_all);
	...
	int I = (4*nfield*s + 6*hnx_all);
	int k = 0;

	for (int j = 0; j < sqt_ncores; ++j){
	    for (int i = 0; i < sqt_ncores; ++i){
	   	copy_in(usub + k*I, u + j*hnx *nx_all + i*hnx, nx, ny, ng, nfield, ncores);
	    	k += 1;
	    }
	} 
...
#pragma omp parallel num_threads(4)
{
	# pragma omp for
	for (int i = 0; i < nc; ++i){ 
            central2d_step(usub + i*I , vsub + i*I, scratchsub + i*I, 
			   fsub + i*I, gsub +i*I,
                           0, hnx+4, hny+4, ng-2,
                           nfield, flux, speed,
                           dt, dx, dy);
            central2d_step(vsub + i*I, usub + i*I, scratchsub + i*I,
			   fsub + i*I, gsub + i*I,
                           1,hnx, hny, ng,
                           nfield, flux, speed,
                           dt, dx, dy);
	...
	for (int j = 0; j < sqt_ncores; ++j){
	    for (int i = 0; i < sqt_ncores; ++i){
	   	copy_out(usub + k*I, u + j*hnx *nx_all + i*hnx, nx, ny, ng, nfield, ncores);
	    	k += 1;
	    }
	} 
	}
